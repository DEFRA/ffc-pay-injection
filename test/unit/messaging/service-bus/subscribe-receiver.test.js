...
describe('subscribeReceiver', () => {
  ...
  test('uses default error handler when none provided', async () => {
    console.error = jest.fn()
    const error = new Error('handler error')

    await subscribeReceiver(receiver, action, undefined, {})
    const errorHandler = receiver.subscribe.mock.calls[0][0].processError
  ...
  test('uses default options when config is undefined', async () => {
    await subscribeReceiver(receiver, action, errorHandler)
